# Spoofing
- A type of attack in which one system entity illegitimately poses as another entity. [^1] #glossary

- It is possible to spoof IP address, MAC address and email address. [^2]
- [IP Spoofing](ip-spoofing.md)
- [Smurfing](smurfing.md)
- [Cross Site Request Forgery](cross-site-request-forgery.md)
## My Notes
[Notes](mynotes/spoofing-notes.md)

[^1]: [ref20231209T101428](references.md#20231209T101428)
[^2]: [ref20231209T130055](references.md#20231209T130055)
